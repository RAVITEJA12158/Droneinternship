    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-slate-800/50" onClick={() => setExpanded(!expanded)}>
        <div className="flex items-center gap-4">
          <span className="text-white font-medium">Shot #{captureSet.shotNumber}</span>
          {captureSet.timestamp && <span className="text-slate-400 text-sm">{new Date(captureSet.timestamp).toLocaleTimeString()}</span>}
          ...
          {captureSet.files.map(f => (
            <div key={f.id} className="flex items-center justify-between px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-slate-300">{f.originalName}</span>
              </div>
